...
# Note: agents move in 8 directions: cost 1 for orthogonal moves, sqrt(2) for diagonal moves,
# and 0 for staying in place (wait). Available heuristics do not use precomputation and are evaluated on-the-fly.

def _precompute_neighbors(grid):
    """Precompute 8-connected neighbors (including wait) for every free cell.
...
def joint_astar(
    ...
    *,
    ...
    - Movement costs: 1 (orthogonal), sqrt(2) (diagonal), 0 (wait)
    ...
    Args:
        ...
        heuristic: 'octile' (recommended, admissible) or 'euclidean' (admissible). No precomputation.
    """
    ...
    # Heuristic (selectable). Default: octile for costs 1 / sqrt(2)
    gx1, gy1 = pos_x[goal1_id], pos_y[goal1_id]
    gx2, gy2 = pos_x[goal2_id], pos_y[goal2_id]

    if heuristic not in ("octile", "euclidean"):
        raise ValueError("heuristic must be one of: 'octile', 'euclidean'")

    if heuristic == "octile":
        # Octile distance: max(dx,dy) + (sqrt(2)-1)*min(dx,dy)
        import math
        SQRT2_MINUS_1 = math.sqrt(2.0) - 1.0
        def h_of(i1, i2, _pos_x=pos_x, _pos_y=pos_y, _gx1=gx1, _gy1=gy1, _gx2=gx2, _gy2=gy2, _s=SQRT2_MINUS_1):
        ...
        # Euclidean distance: hypot(dx, dy)
        import math
        def h_of(i1, i2, _pos_x=pos_x, _pos_y=pos_y, _gx1=gx1, _gy1=gy1, _gx2=gx2, _gy2=gy2, _hypot=math.hypot):
            dx1 = _pos_x[i1] - _gx1
            dy1 = _pos_y[i1] - _gy1
            dx2 = _pos_x[i2] - _gx2
            dy2 = _pos_y[i2] - _gy2
            return _hypot(dx1, dy1) + _hypot(dx2, dy2)

    # Pre-bind movement cost constants and function outside the loop
    import math as _math
    _sqrt2 = _math.sqrt(2.0)
    def step_cost(a, b, _px=pos_x, _py=pos_y, _s2=_sqrt2):
        if a == b:
            return 0.0
        dx = abs(_px[a] - _px[b])
        dy = abs(_py[a] - _py[b])
...
def plot_paths(
    ...
    *,
    ...
    # Vertical crop (rows) if requested
    ...
    # Horizontal crop (columns) if requested
    ...
    def rot_xy(x, y):
        # Use width after crop (W)
        return (W - 1 - y, x)

    def adj_point(x, y):
        # Returns (x',y') if inside crop (rows y0..y1, columns x0..x1), otherwise None
        if (crop_y0 <= x < crop_y1) and (crop_x0 <= y < crop_x1):
            return (x - crop_y0, y - crop_x0)
        return None

    def adj_series(xs, ys):
        # Apply crop on rows (x) and columns (y); out of bounds -> NaN to break the line
        out_x, out_y = [], []
    ...
    # Agent 1 path
    x1 = [p[0][0] for p in path]
    y1 = [p[0][1] for p in path]
    x1, y1 = adj_series(x1, y1)
    if rotate_ccw_90:
        xy_rot = [rot_xy(x, y) if not (np.isnan(x) or np.isnan(y)) else (np.nan, np.nan) for x, y in zip(x1, y1)]
        x1, y1 = zip(*xy_rot) if xy_rot else ([], [])
    plt.plot(y1, x1, 'r-', label='Agent 1 Path')
    ...
    # Agent 2 path
    x2 = [p[1][0] for p in path]
    y2 = [p[1][1] for p in path]
    x2, y2 = adj_series(x2, y2)
    if rotate_ccw_90:
        xy_rot = [rot_xy(x, y) if not (np.isnan(x) or np.isnan(y)) else (np.nan, np.nan) for x, y in zip(x2, y2)]
        x2, y2 = zip(*xy_rot) if xy_rot else ([], [])
    plt.plot(y2, x2, 'b-', label='Agent 2 Path')
    ...
    if show_legend:
        plt.legend()
    ttl = "Joint A* Paths with Distance Constraint"
    plt.title(ttl)
    plt.show()
...
if __name__ == "__main__":
    ...
    # Heuristic selection: 'octile' or 'euclidean'
    heuristic = "euclidean" 
    ...
    crop_x_min = 20      # e.g. 10
    crop_x_max = 90     # e.g. 200
    if path:
